...
class AVLTree {
    ...
    class AVLNode {
        ...
        AVLNode(int countKey, Entry entry) {
    ...
    public AVLNode insert(AVLNode node, int countKey, Entry entry) {
        if (node == null) {
            return new AVLNode(countKey, entry);
        }
        if (countKey < node.countKey) {
            node.left = insert(node.left, countKey, entry);
        } else if (countKey > node.countKey) {
            node.right = insert(node.right, countKey, entry);
        } else {
            node.entries.add(entry);
            return node;
        }
        node.height = 1 + Math.max(getHeight(node.left), getHeight(node.right));
        int balance = getBalance(node);
        if (balance > 1 && countKey < node.left.countKey) {
            return rotateRight(node);
        }
        if (balance < -1 && countKey > node.right.countKey) {
            return rotateLeft(node);
        }
        if (balance > 1 && countKey > node.left.countKey) {
            node.left = rotateLeft(node.left);
